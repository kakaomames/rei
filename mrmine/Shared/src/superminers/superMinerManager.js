...
class SuperMinerManager {
    ...
    debugaAddRandomSuperMinerByRarity(rarity) {
        if (!rarity || typeof rarity.id === "undefined") {
            console.warn("Invalid rarity provided to addRandomSuperMinerByRarity");
            return false;
        }

        if (this.isFull()) {
            console.warn("Cannot add super miner: all slots are full");
            return false;
        }

        let excludeEggs = this.currentSuperMiners.filter(miner => miner.type == superMinerTypes.EGG).length >= 4;
        let filters = { rarity: rarity, isOwned: false };
        let negativeFilters = excludeEggs ? { type: superMinerTypes.EGG } : {};
        
        var miner = this.getRandomBaseSuperMiner(filters, negativeFilters);
        
        if (miner) {
            var addedMiner = this.addSuperMiner(miner);
            if (addedMiner) {
                console.log(`Successfully added ${miner.name} (${rarity.name}) super miner`);
                return addedMiner;
            }
        } else {
            console.warn(`No available ${rarity.name} super miners found that aren't already owned`);
        }
