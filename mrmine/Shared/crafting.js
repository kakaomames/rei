...
function getCraftableBlueprintCount(blueprintCategory = -1) {
    ...
    var blueprintList = getKnownBlueprints(blueprintCategory);
    for (var i in blueprintList) {
        // Only count blueprints that are actually visible/craftable
        if (blueprintCategory == 1) {
            // For drill blueprints, check if they're higher level than equipped
            if (canCraftBlueprint(blueprintList[i].category, blueprintList[i].id, -1) && 
                isDrillBlueprintHigherLevelThanEquippedPart(blueprintList[i])) {
                ++count;
            }
        } else {
            if (canCraftBlueprint(blueprintList[i].category, blueprintList[i].id, -1)) {
...
function getBlueprintNotCraftableReason(category, blueprintId, level = 0) {
    ...
    if (category == 3) {
        var currentStructureLevel = structures[blueprintId].level;
        if (level == 0) {
            level = currentStructureLevel;
        }
    }
...
function craftBlueprint(category, blueprintId, level = 0, ingredients = null) {
    ...
    if (category != craftingCategories.miners && category != craftingCategories.minerLevels) {
...
function getBlueprintById(category, id) {
    ...
    throw "Invalid blueprint" + " category: " + category + " ID: " + id;
}
