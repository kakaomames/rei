class OfferManager {
    ...
    availableRewards = {
        ...
        battleChest: {
            image: battleChestIconClosed,
            name: _("Battle Chest"),
            grant: function (quantity) {
                rewardedChestStorage.storeChests(ChestType.battle, quantity);
            }
        },
        buildingMaterials: {
        ...
        droneFleetConsumable: {
            image: fleetManagerConsumableIcon,
            name: _("Fleet Manager"),
            grant: function (quantity) {
                droneFleetConsumable.droneFleets += quantity;
                newNews(_("You got {0} Drone Fleet!", quantity));
            }
        },
        beacon: {
            image: beaconConsumableIcon,
            name: _("Beacon"),
            grant: function (quantity) {
                revealCaveConsumable.caveReveals += quantity;
                newNews(_("You got {0} Beacon!", quantity));
            }
        },
        healConsumable: {
            image: healConsumable,
            name: _("Medkit"),
            grant: function (quantity) {
                battleHealConsumable.battleHeals += quantity;
                newNews(_("You got {0} Medkit!", quantity));
            }
        },
        equipDrawConsumable: {
            image: recoverConusmable,
            name: _("Card Dispenser"),
            grant: function (quantity) {
                battleDrawConsumable.battleDraws += quantity;
                newNews(_("You got {0} Card Dispenser!", quantity));
            }
        }
    };
    ...
    offerSaveDataMap = {
        ...
        localizedPrice: { index: 13, loadFunction: (value) => value == "" ? null : value },
        localizedCurrency: { index: 14, loadFunction: (value) => value == "" ? null : value },
        localizedPriceFormatted: { index: 15, loadFunction: (value) => value == "" ? null : value }
    };
            ...
            async function (responseJson) {
                ...
                if (response.code == 200) {
                    if (response.data.offer) {
                        await this.updateOffer(response.data.offer);
                    }
    ...
    getRequestParams() {
        return {
            ...
            atomicId: playsaurusSdk.getUserAtomicId()
        }
    }
    ...
    async updateOffer(newOffer) {
        ...
        await this.localizeOfferPrice(newOffer);
        if (platform.registerSinglePurchasePack) {
            platform.registerSinglePurchasePack(this.currentOffer.sku, this.currentOffer.name, this.currentOffer.image.src, this.getOfferPriceCents());
            platform.registerSinglePurchasePack(newOffer.sku, newOffer.name, newOffer.image.src, this.getOfferPriceCents());
    ...
    async localizeOfferPrice(offer) {
        var localizedPrices;
        if (typeof (playsaurusSdk) != "undefined"
            && (localizedPrices = await playsaurusSdk.localizePrices([(offer.price / 100).toFixed(2)], true))) {
            offer.localizedPrice = localizedPrices[0].localized.amount * 100;
            offer.localizedCurrency = localizedPrices[0].localized.currency;
            offer.localizedPriceFormatted = localizedPrices[0].localized.formatted;

            // Temporary fix to bring this in line with purchase pack pricing
            // Eventually the prices for both should be in a component with
            // these fields and the above values should be deprecated
            let localizationResult = localizedPrices[0];
            offer.rawPrice = localizationResult.localized.amount;
            offer.priceCents = localizationResult.localized.amount * 100;
            offer.usdRawPrice = localizationResult.localizedUsd.amount;
            offer.usdPriceCents = localizationResult.localizedUsd.amount * 100;
            offer.currency = localizationResult.localized.currency;
            offer.formattedPrice = localizationResult.localized.formatted;
        }
        else {
            ...
            offer.localizedPrice = offer.price;
            offer.localizedCurrency = "USD";
            offer.localizedPriceFormatted = USDollar.format(offer.price);

            let priceString = (offer.price / 100).toFixed(2);

            offer.rawPrice = priceString;
            offer.priceCents = offer.price;
            offer.usdRawPrice = priceString;
            offer.usdPriceCents = offer.price;
            offer.currency = "USD";
            offer.formattedPrice = "$" + priceString;
        }
        return offer;
    }

    updateStore(timeBetweenUpdates = 1000, maxAttempts = 10, attemptNumber = 0) {
        store.update();
        if (typeof store.get(this.currentOffer.sku, platform.purchasePlatform) == "undefined") {
            if (++attemptNumber < maxAttempts) {
                // Sometimes the store can fail to update, so we retry if the product is missing
                window.setTimeout(this.updateStore.bind(
                    this,
                    timeBetweenUpdates,
                    maxAttempts,
                    attemptNumber
    ...
    completePurchase() {
        ...
        trackEvent_MadePurchase(
            this.currentOffer.sku,
            this.currentOffer.rawPrice,
            this.currentOffer.currency,
            this.currentOffer.usdRawPrice,
            this.currentOffer.usdPriceCents
        );
    ...
    getOfferPriceCents() {
        if(this.currentOffer.localizedPrice)
        {
            return parseInt(this.currentOffer.localizedPrice);
        }
        return this.currentOffer.price;
    }

    getOfferPriceCurrency() {
        if (this.currentOffer.localizedCurrency) {
            return this.currentOffer.localizedCurrency;
        }
        return "USD";
    }

    getOfferPriceString() {
        if (this.currentOffer.localizedPriceFormatted) {
            return this.currentOffer.localizedPriceFormatted;
        }
        else if (this.isEnabled() && this.requireStore && this.isStoreReady() && isMobile()) {
    ...
    set saveState(value) {
        ...
        if (this.currentOffer && platform.registerSinglePurchasePack) {
            ...
            if (typeof (store) != "undefined" && store.update) {
                ...
                if (typeof store.get(this.currentOffer.sku, platform.purchasePlatform) == "undefined") {
                    // Sometimes the store can fail to update, so we retry if the product is missing
                    var waitTimeForUpdate = 5000;
                    window.setTimeout(store.update, waitTimeForUpdate);
                }
            }
        }
