...
class PromoSpecial extends LimitedTimeEvent
{
    constructor()
    {
        super();
        this.id = EVENT_PROMO_SPECIAL;
        this.name = _("Special Promotion");
        this.description = _("A special promotional event is happening! Look out for special clickables that grant tickets!");
        this.numDaysDuration = 10;
        this.displayCountdown = false;
    }

    startTime()
    {
        return new Date(Date.UTC(2025, 9, 10, 12, 0, 0, 0)).getTime();
    }

    activate(eventManager)
    {
        // Enables the promo clickable spawning
    }
...
class LimitedTimeEventManager
{
    ...
    isPromoEvent() {return this.activeEventType == EVENT_PROMO_SPECIAL;}
    ...
    init()
    {
        var eventClasses = [NewYears, Valentines, StPatricks, Easter, AprilFools, Oktoberfest, Halloween, Thanksgiving, BlackFriday, Christmas, PromoSpecial];
