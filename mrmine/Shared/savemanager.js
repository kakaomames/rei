    ...
    if(variableRegistry.length > index && saves[chosenSave].length > index)
    {
        ...
        switch(variableFormat)
        {
            case INTEGER_SAVE_VALUE:
                var parsedValue = parseInt(loadedValue);
                // if (isNaN(loadedValue))
                // {
                //     console.error("Variable '" + variableName + "' is NaN (" + parsedValue + ")");
                //     parsedValue = 0;
                // }
                console.log(variableName + " = " + parsedValue);
                eval(variableName + " = " + parsedValue);
                break;
...
function runPostLoadingFixes()
{
    ...
    currentScientists.choices.forEach((choice, index) =>
    {
        if(choice[0] != undefined && choice[1] != undefined && (choice[0].length !== choice[1].length))
        {
            generateExcavationChoices(index);
        }
    })

    battleEquipsManager.activeEquips.forEach(equip =>
    {
        if(equip.level !== undefined && (equip.level == NaN || equip.level < 1))
        {
            equip.level = 1;
            console.log("Setting level to 1");
        }
        else if(equip.level !== undefined && equip.level > equip.maxLevel)
        {
            //technically this issue has never happened but figured Id add it while Im also adding the check for it being lower
    ...
    for(var i = 0; i < battleInventory.length; i++)
    {
        if(battleInventory[i].length > 0)
        {
            ...
            if(battleInventory[i][6] == 1)
            {
                ...
                battleInventory[i][4]++;
                newNews(weaponStats[battleInventory[i][0]].translatedName + " " + _("Upgraded to Lvl") + " " + battleInventory[i][4], true);
            }
    ...
    {
        //battle equips 
        battleEquipsManager.slots += Math.max(0, Math.floor(((depth - 300) / 100) ** 1.05))

        if(depth < 400) grantDefaultEquips();

        //grant equips based on what the player had in the old system
        battleInventory.forEach((equip, index) =>
        {
            if(equip.length > 0 && index != 7 && index != 9)
            {
                battleEquipsManager.addActiveEquip(index, true);
            }
        });

        //set the level for the old equips
        battleEquipsManager.activeEquips.forEach(equip =>
        {
            let oldEquip = battleInventory.find(e => e.length > 0 && e[0] === equip.id);
            if(oldEquip)
            {
                equip.level = Math.max(1, oldEquip[4]);
            }
        });

        battleEquipsManager.slots = Math.max(battleEquipsManager.slots, battleEquipsManager.activeEquips.length + 2);

        let weaponsLeftToAdd = battleEquipsManager.slots - battleEquipsManager.activeEquips.length;
        let rarityMultiplier = Math.max(1, depthMultiplier() ** .8);
        for(var i = 0; i < weaponsLeftToAdd; i++)
        {
            if(battleEquipsManager.slots > battleEquipsManager.activeEquips.length)
            {
                let rand = battleChestRewardRoller.randFloat() * rarityMultiplier;
                let rarity = equipRarities.common;
                if(rand < .4) rarity = equipRarities.common;
                else if(rand < .75) rarity = equipRarities.uncommon;
                else if(rand < .99) rarity = equipRarities.rare;
                else rarity = equipRarities.legendary;

                let equip = battleEquipsManager.getRandomBaseEquip(rarity);

                if(battleEquipsManager.activeEquips.find(e => e.id === 10 || e.id === 13 || e.id === 14))
                {
                    equip = battleEquipsManager.getRandomBaseEquip(rarity);
                }

                battleEquipsManager.addActiveEquip(equip.id);
            }
        }

        let percentOfLevels = depthMultiplier() / 3;
        battleEquipsManager.activeEquips.forEach(equip =>
        {
            if(equip.level == 1)
            {
                equip.level = Math.max(1, Math.round(battleChestRewardRoller.randFloat() * equip.maxLevel * percentOfLevels));
            }
        });

        battleEquipsManager.slots = Math.max(battleEquipsManager.slots, battleEquipsManager.activeEquips.length + 1);

        battleHealConsumable.battleHeals = 5;
        battleDrawConsumable.battleDraws = 3;
        //battle stats
        ...
        {
            weaponBonus += battleEquipsManager.activeEquips[i].level;
        }
        let hp = (baseHealth + weaponBonus + ((depthBonus + questBonus + buildingBonus) * 4));
        //end old HP logic

        hp /= depthMultiplier() ** 1.5;

        let HealthGemUpgrade = gemUpgradesManager.getGemUpgradeById(0);
        let totalDifficulty = 0;

        for(var i = 0; i < gemUpgradesManager.upgrades.length; i++)
        {
            gemUpgradesManager.upgrades[i].level = 1;
        }

        while(HealthGemUpgrade.levelEffect() < hp)
        {
            totalDifficulty += HealthGemUpgrade.levelDifficulty();
            HealthGemUpgrade.level++;
        }

        for(var i = 1; i < gemUpgradesManager.upgrades.length; i++)
        {
            let upgrade = gemUpgradesManager.upgrades[i];
            let difficultyRemaining = totalDifficulty;

            while(upgrade.levelDifficulty() < difficultyRemaining)
            {
                difficultyRemaining -= upgrade.levelDifficulty();
                upgrade.level++;
            }
        }

        //New scientist system
        for(var i = 0; i < 3; i++)
        {
            if(activeScientists[i].length != 0)
            {
                currentScientists.scientists.push(activeScientists[i]);
                currentScientists.choices.push(excavationChoices[i]);
                currentScientists.excavations.push(activeExcavations[i]);
            }
        }
    ...
    if(previousPlatform !== lastPlatform && typeof (offerManager) !== "undefined")
    {
        offerManager.clearOffer();
    }
...
function registerVariablesToSave()
{
    ...
    registerInteger("chestService.totalBattleChestsOpened");
    registerString("battleStorage.saveState");
    registerString("GemForger.saveString");
    registerString("gemUpgradesManager.saveString");
    registerBoolean("hasSecondCaveSpawned");
    registerBoolean("hasSpawnedFirstOrangeFish");
    registerInteger("revealCaveConsumable.saveState");
    registerInteger("droneFleetConsumable.saveState");
    registerString("timelapsePerSession.saveString");
    registerInteger("worldResources[70].numOwned");
    registerBoolean("hasScrappedFirstRelic");
    registerString("battleEquipsManager.saveState");
    registerInteger("titanTradingPostStructure.level");
    registerInteger("worldResources[69].numOwned");
    registerString("currentScientists.saveString");
    registerInteger("battleChestRewardRoller.seed");
    registerInteger("scientistSlots");
    registerInteger("battleHealConsumable.saveState");
    registerInteger("battleDrawConsumable.saveState");
    registerInteger("nextTradeTimeTitan");
    registerInteger("scrapMinerRoller.seed");
    registerInteger("relicScrapSpent");
    registerInteger("chestCompressor.totalChestsCompressed");
    registerInteger("lastEventPromoClickableClaimTime");
...
function processImport(importSaveData, enforceSingleSave = false)
{
    if(RSc < 3 || enforceSingleSave)
    {
        ...
        if(saveData.length > 0 && cname != "" && (enforceSingleSave || localStorage.getItem(cname) === null || localStorage.getItem(cname) === "" || !isValidSave(cname)))
        {
            ...
            // If a name exists but is invalid, we should clean it up from the registry first
            if(localStorage.getItem(cname) !== null && !isValidSave(cname) && RS.indexOf(cname + "|") !== -1)
            {
                RS = RS.replace(cname + "|", "");
...
function savegame(initiator = null)
{
    ...
    if(isSaveFileLoaded())
    {
        ...
        {
            platform.saveLocalStorageToFile(initiator);
        }
    }
}
...
function loadSaves()
{
    if(localStorage["R"] !== undefined)
    {
        ...
        saves = [];
        var validSaveNames = "";
        var validSaveCount = 0;

        // Check each save for validity and only keep valid saves
        for(var i = 0; i < sids.length; i++)
        {
            if(sids[i] != "")
            {
                if(isValidSave(sids[i]))
                {
                    // Valid save - keep it
                    saves[validSaveCount] = b64_to_utf8(b64_to_utf8(localStorage[sids[i]])).split("|");
                    validSaveNames += sids[i] + "|";
                    validSaveCount++;
                }
                else
                {
                    // Invalid save - don't include in the list
                    console.log("Removing invalid save: " + sids[i]);
                }
            }
        }

        // Update registry with only valid saves
        if(validSaveNames !== localStorage["R"])
        {
            localStorage["R"] = validSaveNames;
            console.log("Updated save registry to remove invalid saves");
        }
        ...
        // Re-initialize sids after cleanup
        sids = localStorage["R"].split("|");
    }
    else
    {
...
function loadGame(x)
{
    ...
    if(x < RSc)
    {
        ...
        sessionstarttime = Math.round(new Date().getTime() / 1000);
    ...
    {
        var errorMessage = `Failed to load save with index ${x} (Save count = ${RSc})`;
        playsaurusSdk.logSaveLoadError(
            "load",
            errorMessage,
            "game_launched"
        )
...
function isValidSave(saveName)
{
    // Check if the save exists in localStorage
    if(localStorage.getItem(saveName) === null || localStorage.getItem(saveName) === "")
    {
        return false;
    }
    ...
    try
    {
        // Try to decode and validate the save data
        var saveData = localStorage[saveName];
        if(b64_to_utf8(saveData).length == 0)
        {
            return false;
        }

        var decodedData = b64_to_utf8(b64_to_utf8(saveData)).split("|");
        var versionInSave = decodedData[9];

        // Check if the version is a valid number
        if(isNaN(versionInSave))
        {
            return false;
        }

        // Ensure the save has enough data fields
    ...
    {
        console.error("Error validating save: " + e);
        return false;
    }
}
...
function createNewGame(name)
{
    ...
    if(RSc < 3 && name != "" && name != "R" && RS.indexOf(name + "|") == -1 && name.indexOf(" ") == -1 && name.indexOf("|") == -1 && name.indexOf(",") == -1 && name.indexOf(";") == -1 && name.indexOf("_") == -1)
    {
        ...
        sids[chosen] = name;
        initAvailableBlueprints();
        savegame("new_game_created"); //new game officially created
...
/**
 * Recovers and decompresses localStorage backup data
 * This function can run independently from the game when LZString is available
 * Returns an object with backup information and save data
 */
function recoverLocalBackup()
{
    // Check if we have LZString available, if not try to load it
    if(typeof LZString === 'undefined')
    {
        console.warn("LZString not available. Loading from CDN...");
        loadLZStringLibrary();

        // If still not available after loading attempt, return error
        if(typeof LZString === 'undefined')
        {
            return {
                success: false,
                error: "LZString library could not be loaded",
                saves: [],
                rawData: null
            };
        }
    ...
    try
    {
        ...
        var compressedBackup = localStorage["backup"];

        // Decompress the backup using LZString
        var decompressedData = LZString.decompress(compressedBackup);

        if(!decompressedData)
        {
            return {
                success: false,
                error: "Failed to decompress backup data - data may be corrupted",
                saves: [],
                rawData: compressedBackup
            };
        }

        // Parse the decompressed data
        var segments = decompressedData.split("|");
        var recoveredSaves = [];
        var saveCount = 0;

        // Process segments in pairs (saveName, saveData)
        for(var i = 0; i < segments.length; i += 2)
        {
            if(i + 1 < segments.length && segments[i] !== "" && segments[i + 1] !== "")
            {
                var saveName = segments[i];
                var saveData = segments[i + 1];

                recoveredSaves.push({
                    name: saveName,
                    data: saveData,
                    dataLength: saveData.length,
                    isValid: validateRecoveredSave(saveData)
                });
                saveCount++;
            }
        }

        // Get backup creation date if available
        var backupDate = localStorage.getItem("backupDate");
        var backupDateFormatted = "Unknown";
        if(backupDate)
        {
            try
            {
                backupDateFormatted = new Date(parseInt(backupDate)).toLocaleString();
            }
            catch(e)
            {
                backupDateFormatted = "Invalid date: " + backupDate;
            }
        }
        ...
        return {
            success: true,
            error: null,
            saves: recoveredSaves,
            saveCount: saveCount,
            backupDate: backupDateFormatted,
            rawCompressedSize: compressedBackup.length,
            rawDecompressedSize: decompressedData.length,
    ...
    catch(error)
    {
        return {
            success: false,
            error: "Error processing backup: " + error.message,
            saves: [],
            rawData: null
        };
    }
...
/**
 * Validates if a recovered save appears to be valid
 * @param {string} saveData - The save data to validate
 * @returns {boolean} - True if save appears valid
 */
function validateRecoveredSave(saveData)
{
    try
    {
        // Check if save data can be base64 decoded (basic validation)
        if(!saveData || saveData.length === 0)
        {
            return false;
        }

        // Try to decode the save data (same validation as isValidSave but simplified)
        var decodedSave = b64_to_utf8(b64_to_utf8(saveData));
        if(!decodedSave || decodedSave.length === 0)
        {
            return false;
        }

        var saveFields = decodedSave.split("|");

        // Check if we have minimum required fields and version field
        if(saveFields.length < 10)
        {
            return false;
        }

        // Check if version field (index 9) is a valid number
        var versionField = saveFields[9];
        if(isNaN(versionField))
        {
            return false;
        }
...
/**
 * Dynamically loads LZString library if not available
 * This allows the function to work independently from the main game
 */
function loadLZStringLibrary()
{
    try
    {
        ...
        {
            attempts++;
            if(typeof LZString !== 'undefined' || attempts > 50) // 5 second timeout
            {
    ...
    {
        console.error("Failed to load LZString library:", error);
    }
}

/**
 * Creates a comprehensive user report for debugging and support
 * Logs organized information to console including save data, backup, and system info
 */
function createUserReport()
{
    console.log("=".repeat(80));
    console.log("==================== MR MINE USER REPORT ====================");
    console.log("=".repeat(80));

    // Report generation time and basic info
    var reportTime = new Date().toISOString();
    console.log("Report Generated:", reportTime);
    console.log("Local Time:", new Date().toLocaleString());

    // User/Platform identification
    var userElectronId = "Unknown";
    try
    {
        userElectronId = typeof UID !== 'undefined' && UID ? UID :
            (typeof platform !== 'undefined' && platform.getUserId ? platform.getUserId() : "Not Available");
    }
    catch(e)
    {
        userElectronId = "Error getting ID: " + e.message;
    }
    ...
    try
    {
        ...
        else if(typeof platform !== 'undefined' && platform.name)
        {
            platformInfo = platform.name;
        }
        else if(typeof isSteam === 'function' && isSteam())
        {
            platformInfo = "Steam";
        }
    ...
    {
        platformInfo = "Error detecting platform: " + e.message;
    }

    console.log("User Electron ID:", userElectronId);
    console.log("Platform:", platformInfo);
    console.log("User Agent:", navigator.userAgent);

    // Game version and save info
    console.log("Game Version:", typeof version !== 'undefined' ? version : "Unknown");
    console.log("Save Registry (R):", localStorage.getItem("R") || "Not Found");
    console.log("Save Count (RSc):", typeof RSc !== 'undefined' ? RSc : "Unknown");
    console.log("Current Save Index:", typeof chosen !== 'undefined' ? chosen : "Unknown");

    console.log("-".repeat(80));

    // Current save export (if available)
    console.log("CURRENT SAVE EXPORT:");
    try
    {
        if(typeof isSaveFileLoaded === 'function' && isSaveFileLoaded() && typeof getB64SaveText === 'function')
        {
            var currentSaveData = getB64SaveText();
            var currentSaveName = typeof sids !== 'undefined' && typeof chosen !== 'undefined' && sids[chosen] ? sids[chosen] : "UnknownSave";
            var exportCode = currentSaveName + "|" + utf8_to_b64(currentSaveData);
            console.log("Export Code Length:", exportCode.length);
            console.log("Export Code:", exportCode);
        }
        else
        {
            console.log("No save currently loaded or export function unavailable");
        }
    }
    catch(e)
    {
        console.log("Error generating save export:", e.message);
    }

    console.log("-".repeat(80));

    // Backup recovery information
    console.log("BACKUP RECOVERY INFORMATION:");
    try
    {
        var backupInfo = recoverLocalBackup();
        if(backupInfo.success)
        {
            console.log("Backup Status: SUCCESS");
            console.log("Backup Date:", backupInfo.backupDate);
            console.log("Save Count in Backup:", backupInfo.saveCount);
            console.log("Compressed Size:", backupInfo.rawCompressedSize, "bytes");
            console.log("Decompressed Size:", backupInfo.rawDecompressedSize, "bytes");
            console.log("Compression Ratio:", Math.round((1 - backupInfo.rawCompressedSize / backupInfo.rawDecompressedSize) * 100) + "%");

            console.log("Saves in Backup:");
            backupInfo.saves.forEach(function (save, index)
            {
                console.log("  " + (index + 1) + ". " + save.name + " - " +
                    (save.isValid ? "VALID" : "INVALID") +
                    " (" + save.dataLength + " chars)");
            });

            if(backupInfo.saves.length > 0)
            {
                console.log("Raw Backup Data (first 500 chars):");
                console.log(backupInfo.rawData.substring(0, 500) +
                    (backupInfo.rawData.length > 500 ? "..." : ""));
            }
        }
        else
        {
            console.log("Backup Status: FAILED");
            console.log("Error:", backupInfo.error);
        }
    }
    catch(e)
    {
        console.log("Error recovering backup:", e.message);
    }

    console.log("=".repeat(80));
    console.log("=================== END USER REPORT ===================");
    console.log("=".repeat(80));
}
